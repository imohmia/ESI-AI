...
def predict_with_logic(input_texts: list[str]):
    """
    Predicts ESI levels for a list of input texts.

    Args:
        input_texts (list): A list of strings containing patient complaints.

    Returns:
        list: A list of dictionaries with input text, original ESI level, and adjusted ESI level.
    """
    try:
        # Tokenize inputs
        ...
        for i, text in enumerate(input_texts):
            adjusted_esi_level = apply_post_processing(text, predicted_esi_levels[i], logits[i])
            results.append({
                "input_text": text,
                "original_esi_level": predicted_esi_levels[i],
                "adjusted_esi_level": adjusted_esi_level
            })
...
# Post-processing function remains unchanged
def apply_post_processing(input_text, predicted_esi_level, logits):
    ...
    # Apply rules based on keywords and confidence
    ...
    if any(keyword in input_text.lower() for keyword in borderline_keywords):
        return min(4, predicted_esi_level)
...
if __name__ == "__main__":
    test_inputs = [
        "Severe chest pain and shortness of breath | Age: 67 | Gender: Male",
        "Mild sore throat and runny nose | Age: 25 | Gender: Female"
    ]
    results = predict_with_logic(test_inputs)
